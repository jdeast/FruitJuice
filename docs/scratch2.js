...
//        return this.sendAndReceive("world.getBlockWithData("+x+","+y+","+z+")")
    ...
    async connect_p({ip, port}) {
        ...
        // --- 1. DNS CACHING LOGIC ---
        if (/[a-zA-Z]/.test(ip) && ip !== "localhost") {
            const cacheKey = "fruitjuice_ip_" + ip;
            const cachedIp = localStorage.getItem(cacheKey);

            try {
                const response = await fetch(`https://dns.google/resolve?name=${ip}&type=A`);
                const data = await response.json();

                if (data.Answer && data.Answer.length > 0) {
                    targetIp = data.Answer[0].data;
                    localStorage.setItem(cacheKey, targetIp);
                } else if (cachedIp) {
                    targetIp = cachedIp;
                }
            } catch (error) {
        ...
        // --- 3. CREATE SOCKET AND WAIT FOR CONNECTION ---
        // We wrap the connection part in a Promise so Scratch waits for it to open
        await new Promise((resolve, reject) => {
            ...
            this.clear();
            const socketUrl = protocol + targetIp + ":" + port;
        ...
        // --- 4. FETCH POSITION AND ROTATION ---
        // Because we use 'await', this runs ONLY after the socket successfully opens above.
        // This replaces your .then() chains and does the exact same thing.
        const posResult = await this.getPosition();
        this.turtle.pos = posResult;

        const rotResult = await this.getRotation();
        this.playerRot = rotResult;
        this.turtle.matrix = this.turtle.yawMatrix(Math.floor(0.5 + rotResult / 90) * 90);
    };

    connect_p_old({ip,port}){
        this.ip = ip;
        this.port = port;
